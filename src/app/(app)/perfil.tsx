...
export default function ProfileScreen() {
    ...
    // Nome vem da sessão; level/vitórias/derrotas vêm da API (com fallback).
    const trainerData = {
        name: user || "Ash",
        level: stats?.level ?? 1,
        battlesWon: stats?.vitorias ?? 0,
        battlesLost: stats?.derrotas ?? 0,
    ...
    useEffect(() => {
        async function loadProfileAndFavorite() {
            try {
                ...
                // Carrega as estatísticas do treinador no backend.
                if (userId) {
                    try {
                        ...
                        <View style={styles.contentLayout}>
                            ...
                            <TouchableOpacity style={styles.rightSection} onPress={() => setShowFavoritePokemon(true)}>
                                <View style={styles.infoCard}>
                                    ...
                                    {renderInfoRow("Level", trainerData.level)}
                                    {renderInfoRow("Vitórias", trainerData.battlesWon)}
                                    {renderInfoRow("Derrotas", trainerData.battlesLost)}
